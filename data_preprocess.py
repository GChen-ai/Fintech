...
train_data['gdr_cd']=train_data['gdr_cd'].astype('category')

test_data['gdr_cd'][test_data['gdr_cd']=='F']=0
test_data['gdr_cd'][test_data['gdr_cd']=='M']=1
test_data['gdr_cd']=test_data['gdr_cd'].astype('category')
#付款方式one hot
temp=pd.get_dummies(train_data['atdd_type'],prefix='atdd_type').astype('category')
train_data[temp.columns]=temp
train_data.drop(columns='atdd_type',inplace=True)

temp=pd.get_dummies(test_data['atdd_type'],prefix='atdd_type').astype('category')
test_data[temp.columns]=temp
test_data.drop(columns='atdd_type',inplace=True)

#学历、教育程度onehot
temp=pd.get_dummies(train_data['deg_cd'],prefix='deg_cd').astype('category')
train_data[temp.columns]=temp
train_data.drop(columns='deg_cd',inplace=True)

temp=pd.get_dummies(test_data['deg_cd'],prefix='deg_cd').astype('category')
test_data[temp.columns]=temp
test_data.drop(columns='deg_cd',inplace=True)

temp=pd.get_dummies(train_data['edu_deg_cd'],prefix='edu_deg_cd').astype('category')
train_data[temp.columns]=temp
train_data.drop(columns='edu_deg_cd',inplace=True)

temp=pd.get_dummies(test_data['edu_deg_cd'],prefix='edu_deg_cd').astype('category')
test_data[temp.columns]=temp
test_data.drop(columns='edu_deg_cd',inplace=True)
test_data['edu_deg_cd_J']=0

#'mrg_situ_cd'有6类，onehot
temp=pd.get_dummies(train_data['mrg_situ_cd'],prefix="mrg_situ_cd").astype('category')
train_data[temp.columns]=temp
train_data.drop(columns='mrg_situ_cd',inplace=True)

temp=pd.get_dummies(test_data['mrg_situ_cd'],prefix="mrg_situ_cd").astype('category')
test_data[temp.columns]=temp
test_data.drop(columns='mrg_situ_cd',inplace=True)
#acdm_deg_cd有8类,onehot
temp=pd.get_dummies(train_data['acdm_deg_cd'],prefix="acdm_deg_cd").astype('category')
train_data[temp.columns]=temp
train_data.drop(columns='acdm_deg_cd',inplace=True)

temp=pd.get_dummies(test_data['acdm_deg_cd'],prefix="acdm_deg_cd").astype('category')
test_data[temp.columns]=temp
...
train_data['tot_ast_lvl_cd']=train_data['tot_ast_lvl_cd'].astype('category')
...
test_data['tot_ast_lvl_cd']=test_data['tot_ast_lvl_cd'].astype('category')
...
train_data['bk1_cur_year_mon_avg_agn_amt_cd']=train_data['bk1_cur_year_mon_avg_agn_amt_cd'].astype('category')
...
test_data['bk1_cur_year_mon_avg_agn_amt_cd']=test_data['bk1_cur_year_mon_avg_agn_amt_cd'].astype('category')
#是否贷款逾期分类特征
...
train_data['job_new']=train_data['job_new'].astype('category')
...
test_data['job_new']=test_data['job_new'].astype('category')
...
train_data['cur_credit_min_opn_dt_cnt_new'] = train_data['cur_credit_min_opn_dt_cnt'].astype('category')
test_data['cur_credit_min_opn_dt_cnt_new'] = test_data['cur_credit_min_opn_dt_cnt'].astype('category')
...
train_data['ovd_30d_loan_tot_cnt_new'] = train_data['ovd_30d_loan_tot_cnt_new'].astype('category')
test_data['ovd_30d_loan_tot_cnt_new'] = test_data['ovd_30d_loan_tot_cnt_new'].astype('category')
#持卡天数/工龄
...
train_data['credit_dt_cnt_year']=train_data['cur_credit_min_opn_dt_cnt']/(train_data['job_year']+1)
test_data['credit_dt_cnt_year']=test_data['cur_credit_min_opn_dt_cnt']/(test_data['job_year']+1)
...
train_data['credit_dt_cnt_start']=train_data['job_year']*365-train_data['cur_credit_min_opn_dt_cnt']
test_data['credit_dt_cnt_start']=test_data['job_year']*365-test_data['cur_credit_min_opn_dt_cnt']

#持卡张数乘以天数
train_data['debit_dt_mul_cnt_start']=train_data['debit_dt_cnt_year']*train_data['cur_debit_cnt']
test_data['debit_dt_mul_cnt_start']=test_data['debit_dt_cnt_year']*test_data['cur_debit_cnt']

train_data['credit_dt_mul_cnt_start']=train_data['credit_dt_cnt_year']*train_data['cur_credit_cnt']
test_data['credit_dt_mul_cnt_start']=test_data['credit_dt_cnt_year']*test_data['cur_credit_cnt']


train_data['credit_dt_mul_cnt_start']=(train_data['credit_dt_mul_cnt_start']-train_data['credit_dt_mul_cnt_start'].min())/(train_data['credit_dt_mul_cnt_start'].max()-train_data['credit_dt_mul_cnt_start'].min())
train_data['debit_dt_mul_cnt_start']=(train_data['debit_dt_mul_cnt_start']-train_data['debit_dt_mul_cnt_start'].min())/(train_data['debit_dt_mul_cnt_start'].max()-train_data['debit_dt_mul_cnt_start'].min())
train_data['credit_dt_cnt_start']=(train_data['credit_dt_cnt_start']-train_data['credit_dt_cnt_start'].min())/(train_data['credit_dt_cnt_start'].max()-train_data['credit_dt_cnt_start'].min())
train_data['debit_dt_cnt_start']=(train_data['debit_dt_cnt_start']-train_data['debit_dt_cnt_start'].min())/(train_data['debit_dt_cnt_start'].max()-train_data['debit_dt_cnt_start'].min())
train_data['debit_dt_cnt_year']=(train_data['debit_dt_cnt_year']-train_data['debit_dt_cnt_year'].min())/(train_data['debit_dt_cnt_year'].max()-train_data['debit_dt_cnt_year'].min())
train_data['credit_dt_cnt_year']=(train_data['credit_dt_cnt_year']-train_data['credit_dt_cnt_year'].min())/(train_data['credit_dt_cnt_year'].max()-train_data['credit_dt_cnt_year'].min())
...
test_data['credit_dt_mul_cnt_start']=(test_data['credit_dt_mul_cnt_start']-test_data['credit_dt_mul_cnt_start'].min())/(test_data['credit_dt_mul_cnt_start'].max()-test_data['credit_dt_mul_cnt_start'].min())

test_data['debit_dt_mul_cnt_start']=(test_data['debit_dt_mul_cnt_start']-test_data['debit_dt_mul_cnt_start'].min())/(test_data['debit_dt_mul_cnt_start'].max()-test_data['debit_dt_mul_cnt_start'].min())

test_data['credit_dt_cnt_start']=(test_data['credit_dt_cnt_start']-test_data['credit_dt_cnt_start'].min())/(test_data['credit_dt_cnt_start'].max()-test_data['credit_dt_cnt_start'].min())

test_data['debit_dt_cnt_start']=(test_data['debit_dt_cnt_start']-test_data['debit_dt_cnt_start'].min())/(test_data['debit_dt_cnt_start'].max()-test_data['debit_dt_cnt_start'].min())

test_data['debit_dt_cnt_year']=(test_data['debit_dt_cnt_year']-test_data['debit_dt_cnt_year'].min())/(test_data['debit_dt_cnt_year'].max()-test_data['debit_dt_cnt_year'].min())

test_data['credit_dt_cnt_year']=(test_data['credit_dt_cnt_year']-test_data['credit_dt_cnt_year'].min())/(test_data['credit_dt_cnt_year'].max()-test_data['credit_dt_cnt_year'].min())
